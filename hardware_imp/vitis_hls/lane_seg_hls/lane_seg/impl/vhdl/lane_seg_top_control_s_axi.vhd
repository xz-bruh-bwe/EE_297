...
entity lane_seg_top_control_s_axi is
generic (
...
end entity lane_seg_top_control_s_axi;
...
architecture behave of lane_seg_top_control_s_axi is
    ...
    constant ADDR_STATUS_I_CTRL   : INTEGER := 16#34#;
    constant ADDR_STATUS_O_DATA_0 : INTEGER := 16#38#;
    constant ADDR_STATUS_O_CTRL   : INTEGER := 16#3c#;
    constant ADDR_MAGIC_DATA_0    : INTEGER := 16#40#;
    constant ADDR_MAGIC_CTRL      : INTEGER := 16#44#;
    ...
    signal int_magic_ap_vld    : STD_LOGIC;
    signal int_magic           : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_MAGIC_DATA_0 =>
                        rdata_data <= RESIZE(int_magic(31 downto 0), 32);
                    when ADDR_MAGIC_CTRL =>
                        rdata_data(0) <= int_magic_ap_vld;
                    when others =>
                        NULL;
                    end case;
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (magic_ap_vld = '1') then
                    int_magic <= UNSIGNED(magic);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (magic_ap_vld = '1') then
                    int_magic_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_MAGIC_CTRL) then
                    int_magic_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
